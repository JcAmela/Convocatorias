import type { Plaza, Grupo } from './tipos.ts';
import { normaliza } from './texto.ts';
import { urgencia, ORDEN_NIVEL, ETIQUETA_AMBITO } from './plazos.ts';
import { type Catalogo, idsFiltroLugar, lugaresTexto } from './catalogo.ts';
import { estaCerca, kmDesde } from './geo.ts';
import { terminosDe } from './oficios.ts';

/**
 * Filtrar, contar y ordenar convocatorias.
 *
 * El catálogo se pasa como argumento: la web se lo inyecta desde
 * `src/lib/filtros.ts` y el servidor se lo pasa a mano, porque evalúa las
 * suscripciones de mucha gente dentro del mismo proceso.
 *
 * Que esto sea una sola copia es lo que hace honesto el aviso por correo: el
 * suscriptor define sus filtros en la web y lo que recibe se calcula con
 * exactamente el mismo código. Dos implementaciones «equivalentes» acabarían
 * mandando cosas que la web no enseña, y al revés.
 */
...
export interface Filtros {
  ...
  ambitos: string[];
  urgencias: string[];
  /** Identificadores de lugar de trabajo, los del catálogo. */
  lugares: string[];
...
/**
 * Texto sobre el que busca la caja de búsqueda, calculado una sola vez por
 * convocatoria. La clave es el objeto `Plaza`, no su id: cuando llegan datos
 * nuevos son objetos nuevos y el caché se rehace solo.
 *
 * Depende del catálogo —de ahí sale el nombre del municipio—, así que sería
 * incorrecto reutilizarlo para el mismo objeto con dos catálogos distintos.
 * No pasa: dentro de una pasada el callejero es uno solo.
 */
const cacheTexto = new WeakMap<Plaza, string>();
function textoBuscable(cat: Catalogo, p: Plaza): string {
  ...
  if (t === undefined) {
    t = normaliza(
      ...
      [p.titulo, p.empleador, lugaresTexto(cat, p), p.nivelEstudios, p.titulacion,
...
export type Criterio = 'q' | 'niveles' | 'contratos' | 'ambitos' | 'urgencias' | 'lugares' | 'cerca' | 'dia';

function cumple(cat: Catalogo, p: Plaza, f: Filtros, criterio: Criterio): boolean {
  switch (criterio) {
    case 'q': {
      if (!f.q.trim()) return true;
      const texto = textoBuscable(cat, p);
    ...
    case 'lugares':
      ...
      return f.lugares.length === 0 || idsFiltroLugar(cat, p).some((id) => f.lugares.includes(id));
    case 'cerca':
      return !f.soloCerca || estaCerca(cat, p, f.desde);
    case 'dia':
...
export function aplica(cat: Catalogo, plazas: Plaza[], f: Filtros, excepto?: Criterio): Plaza[] {
  const criterios = excepto ? TODOS.filter((c) => c !== excepto) : TODOS;
  return plazas.filter((p) => criterios.every((c) => cumple(cat, p, f, c)));
}
...
export function cuenta<T extends string>(
  cat: Catalogo, plazas: Plaza[], f: Filtros, criterio: Criterio, clave: (p: Plaza) => T | null,
): Record<string, number> {
  const base = aplica(cat, plazas, f, criterio);
...
export function cuentaVarias(
  cat: Catalogo, plazas: Plaza[], f: Filtros, criterio: Criterio, claves: (p: Plaza) => string[],
): Record<string, number> {
  const base = aplica(cat, plazas, f, criterio);
  const out: Record<string, number> = {};
  for (const p of base) {
...
export function ordena(cat: Catalogo, plazas: Plaza[], orden: Orden, desde: string | null = null): Plaza[] {
  const copia = [...plazas];
  switch (orden) {
    case 'cercania': {
      /**
      ...
      for (const p of copia) km.set(p.id, kmDesde(cat, p, desde) ?? Infinity);
